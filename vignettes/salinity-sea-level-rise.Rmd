...
v_slr35 <- treat_salinity_35cm %>% rename(S_ppm = S_ppm_SLR_calibrate) %>% 
  calibrate_salinity_model(v_ab_channel_slr35, control = list(trace = FALSE))

# sea level rise of 67 cm
v_slr67 <- treat_salinity_67cm %>% rename(S_ppm = S_ppm_SLR_calibrate) %>% 
...
slr_data <- as.data.frame(matrix(c(ganges_params$param, v_slr35, v_slr67, v_slr54),nrow = 4, byrow = FALSE)) %>% 
  setNames(c("SLR_Baseline_0", "SLR_Dasgupta2014_35","SLR_Dasgupta2014_67","SLR_Akter2016_54")) %>%
  bind_cols(log_var = c("log(a)","log(b)","log(d)","log(C_d)")) %>% 
...
p_slr <- ggplot(slr_data %>% filter(var %in% c("a","b","d"))) + 
  geom_point(aes(SLR_cm, param, color = source, shape = source), size = 2, stroke = 1) + 
  geom_polygon(data = slr_data %>% filter(var == "d", SLR_cm != 35), aes(SLR_cm, param), alpha = 0.5) +
